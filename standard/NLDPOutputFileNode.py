...
class NLDPOutputFileNode(NLDPNode):
    """
    An endpoint node that writes incoming string data to a specified .txt file.
    ...
    def __init__(self, x=0, y=0, **kwargs):
        ...
        layout = [
            {'field_type': constants.FIELD_TYPE_INPUT, 'label': 'Data', 'data_type': constants.DTYPE_STRING},
    ...
    def execute_write(self):
        ...
        if not file_path or not isinstance(file_path, str) or not file_path.lower().endswith('.txt'):
            print(f"Error: Invalid or non-.txt file path: '{file_path}'")
            return
